...
> 소셜 미디어 피드용 콘텐츠 자동 생성 (이미지+텍스트)

## 목적

소셜 미디어 콘텐츠 제작 시간 단축 및 일관성 유지

## 사용자 시나리오

1. 마케터가 '여름 한정 신상품' 주제와 'Instagram' 채널 선택
2. 시스템이 Bedrock Claude로 캡션 3개 생성
3. 동시에 이미지 생성 모델로 상품 배경이미지 3장 생성
4. 생성된 콘텐츠를 바로 SNS에 게시하거나 재검토 후 사용

## 기술 스택

### 백엔드
- **서비스**: FeedGenerationService
...
| 메서드 | 경로 | 인증 |
|--------|------|------|
| POST | /api/marketing/feed-generate | Required (JWT) |
